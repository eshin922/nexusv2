...
async function main() {
  ...
  // Policy 2026-08-22: authority, scope and freshness are the whole control.
  // There is no independence assertion to make here any more.
  rec(
    "gate ACCEPTS the authorization",
    evaluateBelowFloorAuthorization({ authorizations: live, scope, currentFingerprint: fp }).ok,
    true,
  );
  rec(
    "gate REFUSES a moved fingerprint",
    (() => {
      const v = evaluateBelowFloorAuthorization({ authorizations: live, scope, currentFingerprint: fp + "|moved" });
  ...
  rec(
    ...
    (() => {
      const v = evaluateBelowFloorAuthorization({ authorizations: live, scope: { ...scope, tierId: "00000000-0000-0000-0000-000000000000" }, currentFingerprint: fp });
      return v.ok ? "ALLOWED" : v.code;
    })(),
    "NO_AUTHORIZATION",
  );
  rec(
    ...
    (() => {
      const v = evaluateBelowFloorAuthorization({ authorizations: live, scope: { ...scope, quoteVersionNumber: scope.quoteVersionNumber + 1 }, currentFingerprint: fp });
      return v.ok ? "ALLOWED" : v.code;
    })(),
    "NO_AUTHORIZATION",
  );
