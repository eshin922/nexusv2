// Slice 12 Step 10 — CB fixture cleanup.
// Usage: node scripts/cleanup-cb-step10-fixture.mjs '<handoff-json>'
//
...
//   5. HubSpot deal (archive via API — resets stage rollback too)
//
...
if (!argJson) {
  console.error("Usage: node scripts/cleanup-cb-step10-fixture.mjs '<handoff-json>'");
...
try {
  ...
  console.log(`\n[1/5] Scanning NetSuite for SOs with custbody_dps_deal_id='${hubspotDealId}'…`);
  ...
  console.log(`\n[2/5] Deleting Nexus quote row (cascades to tree)…`);
  ...
  // ═════ 3. Nexus project row ═════
  console.log(`\n[3/5] Deleting Nexus project row…`);
  ...
  // ═════ 5. HubSpot deal (archives + drops all stage history / audit trail) ═════
