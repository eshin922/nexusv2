...
// Preview URL for the Step 10 close branch. Auto-generated by Vercel
// on push; the exact hash isn't knowable at script-write time. Users
// can override via env var or replace after first push.
const PREVIEW_URL_BASE = process.env.PREVIEW_URL_BASE
  ?? "https://nexusv2-git-feat-slice-12-step-10-schema-close-eshin922s-projects.vercel.app";
...
async function main() {
  ...
  // ---- 6. Handoff ----
  console.log("\n[6/6] Handoff payload…");
  // Deep-link opens on Mark Accepted sub-tab (4) — that's where CB
  // starts the walk. Sub-tabs 1-3 (preview/send/review) are already
  // 'done' because status='sent'; sub-tab 4 (accepted) is the active
  // frontier; sub-tab 5 (tier/Sales Order) unlocks after accept.
  const url = `${PREVIEW_URL_BASE}/projects/${seeded.projectId}/quotes/${seeded.quoteId}/quote?tab=accepted`;
  const handoff = {
    ...
    smokeTag: SMOKE_TAG,
    markAcceptedTabUrl: url,
  };
  console.log("\n\n================ FIXTURE PROVISIONED ================");
  console.log(JSON.stringify(handoff, null, 2));

  console.log("\n\n============= CB HANDOFF =============\n");
  console.log(`Mark Accepted tab deep link (quote is at status='sent'):`);
  console.log(`  ${url}`);
  console.log(`\nWalk lifecycle: Client Review → Mark Accepted → Sales Order → Complete`);
  console.log(`\nQuote number:  ${seeded.quoteNumber}`);
  console.log(`HubSpot deal:  ${deal.id}  (dealname "${SMOKE_TAG}", stage ${FROM_STAGE_LABEL})`);
  console.log(`  → markAccepted (CB Advance click) pushes to 'Won - In production'`);
