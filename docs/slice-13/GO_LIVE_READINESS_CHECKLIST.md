...
| Nexus-authored Product catalog readiness | HubSpot integration owner + NetSuite administrator | BLOCKED | HubSpot Product zero-price creation proof; HubSpot → NetSuite native Product synchronization evidence | Code/isolated validation sends technical `price=0.00`; live HubSpot read-back and NetSuite propagation remain separate evidence nodes | Live Nexus workflow proves HubSpot zero; a separate controlled sync proves the corresponding NetSuite Item price; Nexus commercial transaction pricing remains authoritative |
