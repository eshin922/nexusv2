...
| Item Group and pricing readiness | Accounting + Operations owners TBD | BLOCKED | Item setup and parity evidence; external Base Price propagation evidence or approved direct-NetSuite alternative | HubSpot Product Unit price → NetSuite Base Price row remains untested | Completed groups approved; transaction price never derives from zero placeholder |
| Nexus → HubSpot Product creation | Nexus product/technical owner | READY | HPP-1 through HPP-3 evidence | None; Nexus-side PVS-018 work is complete | Missing price sends `0.00`; HubSpot returns zero; Nexus stores the HubSpot ID; HubSpot-first and isolation invariants pass |
| HubSpot Product Unit price → NetSuite Base Price row | HubSpot/NetSuite integration owner | IN PROGRESS | Controlled native-sync evidence (HPP-4) | HPP-4 is `UNTESTED`; connector behavior is external to PVS-018 | Controlled evidence proves the corresponding existing/new NetSuite Item Base Price row, or the direct-NetSuite path is approved |
