...
|---|---|---|---|---|
| **REG-1** | Pricing Vendor identity — stable optional HubSpot Company identity + immutable name snapshot persist; legacy read-only; no NetSuite/procurement projection. Claimed **V1 COMPLETE** | **Satisfied** | — | `tests/unit/pricing-vendor-contract.test.ts`; `assembly_leaf_inputs` carries the Company ID + name snapshot; BV-001 §Implementation evidence and §Closeout evidence enumerate the boundary review; VAL-104 recorded. **The one gate claiming completion, and the claim holds.** **AMENDED 2026-08-10 — the browser evidence has never reached a verdict.** VAL-104 is REG-1's browser-level evidence. It was unmeasured (VAL-101 fixture contamination), then failed on a hardcoded runId literal, and now — with that fixed — proves the identity portion (leaf name, derived SKU code, PB-006/PB-007) and stops at a disclosure that **does not exist in the DOM**. A V1 COMPLETE claim is not preserved on evidence that has never produced a verdict. See [CB Suite Health](validation/CB_SUITE_HEALTH.md) **AMENDED AGAIN 2026-08-10 — Satisfied on browser evidence.** VAL-104 now passes end to end from a clean seed (22.1s), proving the full contract: one Pricing Vendor already selected · another eligible vendor found by search · the operator switches to it · the new vendor persists into the governed snapshot · the dormant Pricing Date stays absent. Six migration artifacts were found and fixed along the way and **none was a product defect**. Caveat recorded: the verdict comes from an isolated run of this scenario — in a full-suite run VAL-101 fixture contamination still blocks the file, which is separately classified. See [CB Suite Health](validation/CB_SUITE_HEALTH.md) |
