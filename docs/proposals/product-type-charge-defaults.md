...
- **No NetSuite item column, ever.** `other_service` **and `otc_testing`**
  choose their item per line, frozen at send — `PER_LINE_DESTINATIONS` at HEAD.
  A column here would be a second answer, sitting in Settings looking
  authoritative while the frozen per-line selection is what posts. *(An earlier
  draft said testing took a firm-wide mapping, from a stale migration comment.
  Corrected and verified executing — see
  `../business-validation/fee-charge-decisions.md` §1.)*
