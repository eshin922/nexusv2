...
> ⚠ **Sub-tabs 4 and 5 in this document are SUPERSEDED by R9 / R9.1** — see
> `docs/r9-data-source-map.md`. In particular `customer_signal_tier_id` (items below and
> summary item 3) was our notation, **not** a real column: the canonical pair is
> `customer_accepted_tier_id` (choice) and `accepted_tier_id` (commitment), both already
> in `schema.ts`. Sub-tabs 1–3 remain authoritative here.
