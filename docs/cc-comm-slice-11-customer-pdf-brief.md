...
| **E** | `detail_level` not on type | **Fork 1 — both persist** (post-Step-1 amendment 2026-06-29). Step 1 §0.5 catch #77 confirmed the cited precedent doesn't exist: `pdfLayout` is render-time-only on schema today (no live column, no snapshot, no enum, no sendQuote write). `pdfLayout` is *retrofitted* into the snapshot fleet alongside `detail_level` so every sent PDF reproduces identically from frozen state. **Migration 0022 lands in Step 4** (adapter contract, before send-action layer touches the columns) — `CREATE TYPE pdf_layout AS ENUM ('tier_table','single_tier')` + `CREATE TYPE detail_level AS ENUM ('itemized','turnkey_only')` + add both nullable columns to `quotes` **+ `include_spec_addendum boolean` column** (post-Step-3 amendment 2026-06-29; §0.5 catch #79 — the impl-6 toggle was banked "session-transient until PDF generation lands; lift to per-quote column then" — Slice 11 IS that landing). No backfill — adapter defaults NULL → `'tier_table'` / `'itemized'` for legacy sent quotes; `include_spec_addendum` NULL → impl-6 default (addendum on when meaningful content exists per existing `hasMeaningfulContent` guard). Wiring: `sendQuote` writes all three inside the existing DEC-7 snapshot transaction + audit `diff_json.snapshots` carries all three; adapter read path `isSent ? quote.{col} : (toggle UI state ?? default)`; `preview-toolbar.tsx` toggle writes live value for drafts, renders read-only for sent+; `CustomerView` type adds `detailLevel: 'itemized' \| 'turnkey_only'` + `includeSpecAddendum: boolean`. This is the **only migration in Slice 11**. |
...
- `sendQuote` calls `renderToBuffer`, uploads to Supabase Storage bucket `quotes-pdfs/<quoteId>/<send-event-id>.pdf` (service-role key), writes the signed URL to `quotes.pdfUrl` (schema column already exists, currently unwired). **The persisted buffer includes addendum pages when the `include_spec_addendum` toggle is on** (post-Step-3 amendment 2026-06-29; one `<Document>`, one buffer — pricing pages + N addendum pages assembled by the multi-page seam built in Step 3).
...
No email infrastructure (SMTP/Resend/etc.) · no tokenized or public customer URL · no separate customer-facing route · no HubSpot stage push (Slice 12 / Quote umbrella) · no NetSuite SO push (Slice 12) · the 4-sub-tab Quote-umbrella restructure (Slice 12) · `retailBenchmark` renderer (**drop the field from `CustomerViewSku` per F1.4 / Track-6 — it's present in the bundle, rendered nowhere; remove the type promise**).

**Spec addendum is IN SCOPE** (post-Step-3 amendment 2026-06-29; §0.5 catch #79). The addendum is already shipped (impl-6 — `pdf-addendum.tsx` + `AddendumToggle` + `loadQuoteAddendum` + `QuoteAddendumData` + multi-page QuoteHost wiring all in `src/components/pdf/`) but **preview-only**; "actual PDF file generation deferred to impl-7/follow-up" — Slice 11 IS that deferred generation. Option A locked by Edward ("we include specs all the time when requested"): the addendum is **ported verbatim** (Pattern 30) from its shipped `.a1v2-*` source to react-pdf and included in the generated PDF, gated by the `include_spec_addendum` snapshot column (added to migration 0022 per §3 Q-E amendment). Data layer already exists; this is render-tree port only (Step 3b). No CD coherence audit — any visual dissonance vs. pricing pages banks as v1.x and does not gate Slice 11.
...
3. **`.pp-*` → StyleSheet translation + component tree port** (§2.3) — verbatim against CD source — **+ multi-page `<Document>` seam** (post-Step-3 amendment 2026-06-29). The generated PDF + QuoteHost preview assemble as one react-pdf `<Document>` of pricing page(s) + N addendum pages; build the seam architecture in Step 3 even though Step 3b fills the addendum pages.
3b. **Addendum verbatim port** (post-Step-3 amendment 2026-06-29; §0.5 catch #79). Port the shipped `.a1v2-*` addendum (`AddendumPage`, per-ASY blocks, the 3 leaf-block variants — typed / placeholder / untyped) from DOM to react-pdf against the Step-3 seam. **Render-tree port only** — data loader (`loadQuoteAddendum` / `QuoteAddendumData`) shipped in impl-6 and stays untouched. Same translate-don't-improvise discipline as pricing port. No CD coherence audit.
4. **Adapter contract** (§3 A–G) + type changes (`email`, `detailLevel`, `includeSpecAddendum`; drop `retailBenchmark`) + real `recommendedTierIdx` (§6) + **migration 0022** (per amendment E — `pdf_layout` + `detail_level` PG enums + columns on `quotes` + `include_spec_addendum boolean`; sendQuote writes all three inside DEC-7 snapshot transaction).
...
8. **CB smoke guide + Pattern 27 manifest wrap.** CB walks the 12-cell matrix (3 states × 2 layouts × 2 detail levels) **+ addendum scenarios ㉓–㉘ analog** (post-Step-3 amendment 2026-06-29 — `include_spec_addendum` on/off × leaf-block variants typed/placeholder/untyped) — Edward-ready handoff (resolved URLs, seed, walk script).

**Merge gate:** CB smoke walk per standing requirement. **CD-fidelity check (Designer audit) is the load-bearing acceptance gate** for a Pattern-30 port: the ported structure matches CD's committed source — segmented turnkey cards are cards, recommended bracket brackets, tiers-as-flex-columns hold; per-ASY addendum blocks with nested leaf sub-blocks, the 3 leaf-block variants render correctly, the `--` empty-field treatment holds, page footer meaningful with addendum on. A content walk passes an improvised port; fidelity is the gate that catches improvisation.
