...
| Q | Item | Disposition |
|---|---|---|
| **A** | `customer.email` missing from `CustomerViewCustomer` | **Add `email` to the type.** Real customer-visible field; parties block renders it. |
...
| **E** | `detail_level` not on type | **Add `detailLevel: "itemized" \| "turnkey_only"` to `CustomerView`**, mirroring `pdfLayout`. Sent-time parameter; snapshots the same way. |
...
0. **Smoke gate** (§1) — throwaway branch, ~1hr. Green → proceed; red → escalate.
1. **Kickoff + Pattern 22 §0.5 schema verification + Pattern 45 boundary plan.** Promote `@react-pdf/renderer@^4.5.0` to runtime dep.
...
4. **Adapter contract** (§3 A–G) + type changes (`email`, `detailLevel`; drop `retailBenchmark`) + real `recommendedTierIdx` (§6).
5. **Data wiring** (§5 F1.5/F1.3/F1.2) + totals derivation from bundle.
6. **Persistence** (§7) — Storage bucket, `pdfUrl` writer, `sendQuote` integration.
7. **Boundary-guard build assertion** (§8) + re-grep.
