...
| ~~**A-2**~~ | **SETTLED 2026-08-10** — one capability, projected into three read sites; queries proven against production with the cost measured (326-357ms, so the loader stays off the bundle's hot path); `NodeCandidate` gained `provenanceKey` + `origin`, closing the model half. Real attribution resolving on packaging, production, worksheet freight and firm margin policy. Seven input types with no audit rows recorded as F5 findings. See [`a2-provenance-evidence.md`](rehearsals/a2-provenance-evidence.md). Original scope preserved below | Nexus engineering, then Edward | A written query per input type, proven against production, with the cost measured. Inputs with no record are a finding (F5, §6.3). **Scope extended 2026-08-09:** `NodeCandidate` is `{ label, value, chosen, unavailableReason }` — a resolution's chosen rung has nowhere to carry provenance even once it is known. Surfaced by the trace overlay on `quote-wide/target-margin`, whose chosen rung is a firm setting a person entered. This is a MODEL gap, not a query gap: writing the query does not close it. Closing it means either a provenance slot on `NodeCandidate` or an `origin` operand beneath the chosen rung — R10's `Resolution` assumes the former, rendering `node.chosen.origin`. Asserted as-is in `tests/unit/pricing-trace-projection.test.ts` so the record cannot rot |
...
| ~~**A-6**~~ | **CLOSED 2026-08-09**, on the governing rule rather than the inventory list. Every classified duplicate eliminated or dispositioned; consumer cutovers landed across the Costs header, Packaging, worksheet Freight, effective target, and the Pricing classifier (committed **and** preview). See [`gate-1b-derivation-inventory.md`](gate-1b-derivation-inventory.md) | — | Closing on the *rule* is deliberate: the inventory grew twice during remediation, because a sweep cannot find a duplicate that agrees with what it duplicates. One violation stays open **by decision** — the per-tier margin twin, carved so it remains independently attributable from the quote-wide S-7 move |
...
| ~~**S-4**~~ | Provenance can be resolved per input without a query per node | **CONFIRMED 2026-08-10** — ONE `DISTINCT ON` for the whole quote, bounded to ids derived from it and served by `audit_log_entity_idx`. 326-357ms against production for 28-41 entity ids; measured rather than assumed, which is why the loader stays off the bundle's hot path | An N-node graph becomes N queries. This is the most likely performance failure and is untested |
