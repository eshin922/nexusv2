...
/**
 * Revalidate every page in the quote tree, including the Setup review checkpoint. Call from any action that
...
export function revalidateQuoteTree(projectId: string, quoteId: string) {
  ...
  revalidatePath(`${base}/setup/review`);
  ...
  console.log(
    `[revalidate-timing] revalidateQuoteTree 9 paths ${Date.now() - revalidateStart} ms quote=${quoteId}`,
  );
}
