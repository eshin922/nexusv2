...
function ShipmentEdit({ shipment, memberships, components, pending, submit }: any) {
  ...
  return <details className="fr-edit-disclosure"><summary>Edit shipment</summary>
    <form action={submit} className="fr-editform">
      ...
      {/* OD-001 V1 — choice removed; the shipment's OWN persisted value is echoed
    back so an edit cannot rewrite it. A legacy `pass_through` row keeps
    `pass_through`: this change removes an operator affordance, it does not
    migrate data. */}
...
function destinationFreightPerUnit(destination: any, tier: Tier, workbook: FreightWorkbook) {
  ...
  return computeShipmentContribution({
    // The SHIPMENT's own per-unit figure, undivided. The equal split across
    // members happens where freight reaches a product's cell; the freight
    // worksheet shows what the shipment costs, which is the number the
    // operator entered and is comparing destinations on.
    memberCount: 1,
