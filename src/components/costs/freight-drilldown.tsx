...
function DestinationRow({ destination, shipment, destinations, selected, tiers, workbook, editable, pending, open, toggle, submit }: any) {
  ...
  return <div className={`fr-dest${isSelected && destinations.length > 1 ? " sel" : ""}`} ref={rowRef}>
    ...
    {open && <div className="fr-entry"><div className="fr-grid"><div className="fr-elab">freight type · per break</div>{cells.map(({ tier, row }) => <div className="fr-cell" key={tier.id}>{row ? <select className="fr-in txt" data-break-field name={`mode:${tier.id}`} defaultValue={row.mode ?? ""} disabled={!editable} onChange={saveBreaks} aria-label={`Freight type · ${tier.label}`}><option value="">Not set</option>{FREIGHT_LEG_MODES.map((value) => <option key={value} value={value}>{enumLabel(value)}</option>)}</select> : <span className="cbm">—</span>}</div>)}</div><div className="fr-grid"><div className="fr-elab">item / description</div>{cells.map(({ tier, row }) => <div className="fr-cell" key={tier.id}>{row ? <input className="fr-in txt" data-break-field name={`shipmentNote:${tier.id}`} defaultValue={row.shipmentNote ?? ""} placeholder="4 pallets" disabled={!editable} onBlur={saveBreaks} aria-label={`Item or description · ${tier.label}`}/> : <span className="cbm">—</span>}</div>)}</div>{editable && <DestinationEdit destination={destination} pending={pending} submit={submit(updateFreightDestination)}/>}</div>}
  </div>;
}
