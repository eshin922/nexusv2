...
function DestinationRow({ destination, shipment, destinations, selected, tiers, workbook, editable, busy, open, toggle, submit }: any) {
  ...
  return <div className={`fr-dest${isSelected && destinations.length > 1 ? " sel" : ""}`} ref={rowRef}>
    ...
    {open && <div className="fr-entry"><div className="fr-grid"><div className="fr-elab">freight type · per break</div>{rows.map((row: any) => <div className="fr-cell" key={row.id}><input className="fr-in txt" data-break-field name={`mode:${row.tierId}`} defaultValue={row.mode ?? ""} placeholder="Domestic LTL" disabled={!editable} onBlur={saveBreaks}/></div>)}</div><div className="fr-grid"><div className="fr-elab">item / description</div>{rows.map((row: any) => <div className="fr-cell" key={row.id}><input className="fr-in txt" data-break-field name={`shipmentNote:${row.tierId}`} defaultValue={row.shipmentNote ?? ""} placeholder="4 pallets" disabled={!editable} onBlur={saveBreaks}/></div>)}</div>{editable && <DestinationEdit destination={destination} pending={busy(`editDestination:${destination.id}`)} submit={submit(updateFreightDestination, `editDestination:${destination.id}`)}/>}</div>}
  </div>;
}
